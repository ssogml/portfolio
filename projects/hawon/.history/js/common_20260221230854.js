...
window.addEventListener("scroll", () => {
  const r = bestTop.getBoundingClientRect();
  const inView = r.top < window.innerHeight && r.bottom > 0; // 화면에 걸쳐있으면 true
  document.body.classList.toggle("invert-gotop", inView);
});